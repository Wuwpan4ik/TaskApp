			...
			<div class="task__main">
				<div class="task__form">
					...
					<div class="form__block">
						...
						<form action="readyAll.php" method='POST'>
						...
						function addTask($desc, $status) {
							$task = "<li class=\"task__item\">
										<div class=\"task__item-main\">
											...
											<div class=\"task__item-circle\" style='{$status}'></div>
										</div>
									</li>";
							echo $task;
						}
						require_once('./vendor/connect.php');
						//Находим пользователя (id)
						$user = ($db -> query("SELECT * FROM `users` WHERE login = '" . $_SESSION['login'] . "'"))[0]['id'];
						$tasks = ($db -> query("SELECT `description`, `status` FROM `tasks` WHERE user_id = '". $user ."'"));
						foreach($tasks as $task) {
							addTask(htmlspecialchars($task['description']), htmlspecialchars($task['status']));
						}
					?>	
				</ul>
			</div>
